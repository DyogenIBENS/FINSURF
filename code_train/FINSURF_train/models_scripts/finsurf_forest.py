...
def _generate_sample_indices(random_state, y):
    ...
    if True:
        ...
        uniq_class, counts = np.unique(y, return_counts=True)
        
        # We will sample min_size * N_class. Each class is sampled with
        # replacement.
        min_size = min(counts)
        y_indices = np.arange(0,len(y))

        sample_indices = []
        for cl in uniq_class:
            cl_sampled = random_instance.choice(y_indices[(y==cl).flatten()], size=min_size)
            sample_indices.append(cl_sampled)
...
def _generate_unsampled_indices(random_state, y):
    """Private function used to forest._set_oob_score function."""
    n_samples = len(y)
    sample_indices = _generate_sample_indices(random_state, y)
